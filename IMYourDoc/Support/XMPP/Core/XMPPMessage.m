...
-(BOOL) isGroupchat{
    return ( [[self attributeStringValueForName:@"type"] isEqualToString:@"groupchat"]);
}
// For BroadCast
/* Headline */
-(BOOL) isHeadline{
    return ( [[self attributeStringValueForName:@"type"] isEqualToString:@"headline"]);
}

-(BOOL) isEventTypeMemberAdd{
    
    NSArray *eventItems = [self elementsForName:@"event"];
    
    if (eventItems.count == 1) {
        DDXMLElement *eventItem = eventItems.firstObject;
        
        NSString *strType = [[eventItem attributeForName:@"type"] stringValue];
        
        if (strType && [strType isEqualToString:@"add"]){
            return YES;
        }
    }
...
-(BOOL) isEventTypeMemberRemoved{
    ...
    if (eventItems.count == 1) {
        ...
        if (strType && [strType isEqualToString:@"removed"]){
            return YES;
        }
    }
...
-(NSDictionary*)jsonMessageBody
{
    
     return [NSJSONSerialization JSONObjectWithData:[[[self elementForName:@"body"] stringValue] dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
}
