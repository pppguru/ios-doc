...
- (void)updateWithModel:(ProfileResponseModel *)model
{
    NSString *jidStr = [NSString stringWithFormat:@"%@@imyourdoc.com", model.username];
    XMPPJID *jid = [XMPPJID jidWithString:jidStr];
    
    if (jid == nil)
    {
        XMPPLogVerbose(@"XMPPUserCoreDataStorageObject: invalid item (missing or invalid jid): %@", model);
        return;
    }
    
    self.jid = jid;
    self.nickname = model.name;
