...
@implementation  RoomMembers

@synthesize jid,role,nickname;

+(instancetype) roomMembers:(XMPPJID *)jid
{
    RoomMembers * member=[RoomMembers new];
...
- (void)handleFetchMembersListResponse:(XMPPIQ *)iq withInfo:(id <XMPPTrackingInfo>)info
{
	if ([[iq type] isEqualToString:@"result"])
	{
        ...
        {
            RoomMembers * membr=[[self.members filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"jid=%@",[XMPPJID jidWithString:[member attributeStringValueForName:@"jid"]]]] lastObject];
            
            if(membr==nil)
            {
                membr=[[RoomMembers alloc] init];
                
                membr.jid=[XMPPJID jidWithString:[member attributeStringValueForName:@"jid"]];
                
                [self.members addObject:membr];
            }
            membr.role=[member attributeStringValueForName:@"affiliation"];
            membr.nickname=[member attributeStringValueForName:@"nick"];
        }
...
-(void)addMembers:(NSArray *)members
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [iq addAttributeWithName:@"to" stringValue:[self.roomJID bare]];
        
        NSString * iqID=[NSString stringWithFormat:@"%@",[xmppStream.myJID bare]];
        ...
        for (XMPPUserCoreDataStorageObject *user in members)
        {
            NSXMLElement * item=[NSXMLElement elementWithName:@"item"];
            if([[user.jid bare] length]==0)
                continue;
            
            if(![user.jid.domain isEqualToString:@"imyourdoc.com"])
            {
                user.jid=[XMPPJID jidWithUser:user.jid.user domain:@"imyourdoc.com" resource:nil];
            }
            [item addAttributeWithName:@"jid" stringValue:[user.jid bare]];
            
            if([user respondsToSelector:@selector(role)])
                [item addAttributeWithName:@"affiliation" stringValue:[user valueForKey:@"role"]];
            else
             [item addAttributeWithName:@"affiliation" stringValue:@"member"];
        ...
        [responseTracker addID:iqID target:self selector:@selector(handleAddMembers:trackerInfo:) timeout:60];
...
-(void)addMember:(RoomMembers *) member
{
...
-(void)deleteMember:(RoomMembers *) member
{
    member.role=@"none";
...
-(void)fetchRoomInfo
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSString * iqID=[xmppStream generateUUID];//[NSString stringWithFormat:@"%@_%lld",[xmppStream.myJID bare],(long long)[[NSDate date] timeIntervalSince1970]];
        [iq addAttributeWithName:@"id" stringValue:iqID];
        ...
        [responseTracker addID:iqID target:self selector:@selector(handleInfo:trackerInfo:) timeout:60];
...
-(void)requestAdminRemove
{
    
    
}

-(void)handleInfo:(id) object trackerInfo:(id<XMPPTrackingInfo>) info
{
    [multicastDelegate xmppRoom:self didfetchInfo:object];
}
-(void)handleAddMembers:(id) object trackerInfo:(id<XMPPTrackingInfo>) info
{
    
    [self fetchModeratorsList];
    
    [multicastDelegate xmppRoom:self didAddMember:object];
}
