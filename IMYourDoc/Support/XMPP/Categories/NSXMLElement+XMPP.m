...
-(void)addProperty:(NSString *)name value:(NSString *) value type:(NSString *)type
{
    
    NSXMLElement * properties=[self elementForName:@"properties" xmlns:@"http://www.jivesoftware.com/xmlns/xmpp/properties"];
    if(properties==nil)
    {
        properties=[NSXMLElement elementWithName:@"properties" xmlns:@"http://www.jivesoftware.com/xmlns/xmpp/properties"];
        
        [self addChild:properties];
    }
    
    
    // [properties removeElementForName:name];
    ...
    [valueElement setStringValue:value];
...
-(NSXMLElement *)getPropertyForName:(NSString *)name
{
    NSXMLElement * properties=[self elementForName:@"properties" xmlns:@"http://www.jivesoftware.com/xmlns/xmpp/properties"];
    
    
    NSArray * props=[properties elementsForName:@"property"];
    
    for (NSXMLElement * element in props) {
        
        if([[[element elementForName:@"name"] stringValue ] isEqualToString:name])
            return element;
    }
